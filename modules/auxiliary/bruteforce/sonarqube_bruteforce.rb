...
module Metasploit
  module Framework
    module LoginScanner
      class SonarQube < ::Metasploit::Framework::LoginScanner::HTTP
        def attempt_login(credential)
          ...
          begin
            ...
            req = cli.request_cgi({
              'method' => 'POST',
              'uri' => uri + (uri[-1] == '/' ? '' : '/')  + '/api/authentication/login',
              'vars_post' => {
...
class MetasploitModule < Msf::Auxiliary
  ...
  def initialize
    super(
        'Name'           => 'SonarQube Bruteforce',
        'Description'    => %q(
        This module attempts to bruteforce credentials on a SonarQube instance.
  ...
  def run_host(ip)
    ...
    scanner = Metasploit::Framework::LoginScanner::SonarQube.new(
