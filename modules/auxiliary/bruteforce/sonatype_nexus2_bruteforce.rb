...
module Metasploit
  module Framework
    module LoginScanner
      class SonatypeNexus2 < ::Metasploit::Framework::LoginScanner::HTTP
        def attempt_login(credential)
          ...
          begin
            ...
            req = cli.request_cgi({
              'uri' => uri + (uri[-1] == '/' ? '' : '/') + 'service/local/authentication/login',
              'headers' => {
                'Authorization' => 'Basic ' + Rex::Text.encode_base64(credential.public + ':' + credential.private)
              }
            })
            ...
            if res && res.code == 200
              ...
              result_opts.merge!(status: ::Metasploit::Model::Login::Status::UNABLE_TO_CONNECT, proof:res)
            end
          rescue Exception => e
...
class MetasploitModule < Msf::Auxiliary
  ...
  def initialize
    super(
        'Name'           => 'Sonatype Nexus 2 Bruteforce',
        'Description'    => %q(
        This module attempts to bruteforce weak credentials on a Sonatype Nexus v2 instance.
...
dependency packages.
  ...
  def run_host(ip)
    ...
    scanner = Metasploit::Framework::LoginScanner::SonatypeNexus2.new(
