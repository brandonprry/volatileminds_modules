...
module Metasploit
  module Framework
    module LoginScanner
      class NagiosXI < ::Metasploit::Framework::LoginScanner::HTTP
        def attempt_login(credential)
          ...
          begin
            ...
            req = cli.request_cgi({
              'uri' => uri + (uri[-1] == '/' ? '' : '/') + 'login.php'
            })

            res = cli.send_recv(req)

            token = $1 if res.body =~ /var nsp_str = "(.*?)";/
            ...
            if res.code == 302 && res.headers['Location'] == 'index.php'
...
class MetasploitModule < Msf::Auxiliary
  ...
  def initialize
    super(
        'Name'           => 'Nagios Bruteforce',
        'Description'    => %q(
        This module attempts to bruteforce weak credentials on Nagios instances.
...
assets.
  ...
  def run_host(ip)
    ...
    scanner = Metasploit::Framework::LoginScanner::NagiosXI.new(
