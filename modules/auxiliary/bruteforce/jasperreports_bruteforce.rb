...
module Metasploit
  module Framework
    module LoginScanner
      class JasperReports < ::Metasploit::Framework::LoginScanner::HTTP
        def attempt_login(credential)
          ...
          begin
            ...
            req = cli.request_cgi({
              'uri' => uri + (uri[-1] == '/' ? '' : '/')  + 'j_spring_security_check',
              'method' => 'POST',
              'vars_post' => {
            ...
            if res && res.headers['Location'] =~ /loginsuccess/
...
class MetasploitModule < Msf::Auxiliary
  ...
  def initialize
    super(
        'Name'           => 'JasperReports Bruteforce',
        'Description'    => %q(
        This module attempts to bruteforce weak credentials on JasperReports instances.
  ...
  def run_host(ip)
    ...
    scanner = Metasploit::Framework::LoginScanner::JasperReports.new(
