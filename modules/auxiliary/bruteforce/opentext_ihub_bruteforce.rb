...
module Metasploit
  module Framework
    module LoginScanner
      class OpenTextiHub < ::Metasploit::Framework::LoginScanner::HTTP
        def attempt_login(credential)
          ...
          begin
            ...
            req = cli.request_cgi({
              'method' => 'POST',
              'uri' => uri + (uri[-1] == '/' ? '' : '/') + 'login.do',
              'vars_post' => {
                'loginpostback' => true,
                'userid' => credential.public,
            ...
            if res && res.code == 302 && res.headers['Location'] =~ /dashboard/
...
sensitive information such as network credentials.
  ...
  def run_host(ip)
    ...
    scanner = Metasploit::Framework::LoginScanner::OpenTextiHub.new(
